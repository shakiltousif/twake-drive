...
function safeToString(obj) {
  ...
  if (obj instanceof IncomingMessage) return `Request<${JSON.stringify(obj.method)} ${obj.url}>`;
  if (obj instanceof ServerResponse)
    //obj.setHeader && obj.socket && obj.req)
  ...
  if (description.length > maxLen + 3) description = description.substr(0, maxLen) + "...";
  if (description === "{}" && className) return className;
  return description;
}
  ...
  if (Array.isArray(kind))
    if (Array.isArray(result))
      return result.map(item => {
        if (!item || (typeof item != "object" && typeof item != "function")) return item;
...
export function makeCallDebuggingProxy<T extends object>(
  kind: KindType,
  obj: T,
  resultWrappers?: WrapMethods,
): T {
        ...
        return function (...args) {
          ...
          try {
            ...
            if (result?.then) {
              ...
              if (wrapper) return result.then(result => wrapResult(wrapper, result));
            } else reporter.callReturned(result);
            if (wrapper) return wrapResult(wrapper, result);
            return result;
          } catch (err) {
